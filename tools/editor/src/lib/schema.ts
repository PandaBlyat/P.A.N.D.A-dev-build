...
export const OUTCOME_SCHEMAS: CommandSchema[] = [
  ...
  {
    name: 'panda_task_dead_drop_stash',
    label: 'Task: Dead Drop (Specific Stash)',
    description: 'Player must deposit item at a specific named stash box; item removed on deposit',
    category: 'Tasks',
    helpText: 'The player must bring the item to the exact stash box and be within range. The item is taken from inventory on completion. The stash is marked on the PDA map via the task objective.',
    examples: ['panda_task_dead_drop_stash:dark_valley_treasure_1:device_pda_3:600:3:4'],
    params: [
      { name: 'stash_name', type: 'stash_name', required: true, label: 'Stash Location', editor: { kind: 'stash_picker' } },
      { name: 'item', type: 'item_section', required: true, label: 'Item to Deposit', editor: ITEM_PICKER_PANEL_EDITOR },
      { name: 'timeout', type: 'number', required: true, label: 'Timeout (s)', min: 30 },
      { name: 'success_turn', type: 'number', required: true, label: 'Success Turn', min: 2, editor: TURN_REFERENCE_EDITOR },
      { name: 'fail_turn', type: 'number', required: true, label: 'Fail Turn', min: 2, editor: TURN_REFERENCE_EDITOR },
  ...
  {
    name: 'panda_task_fetch_from_stash',
    label: 'Task: Fetch from Stash',
    description: 'Item pre-spawned inside specific stash box; player must loot and keep it',
    category: 'Tasks',
    helpText: 'The item is placed inside the stash box when the task starts. The stash is marked on the PDA map. The player keeps the item on success.',
    examples: ['panda_task_fetch_from_stash:dark_valley_treasure_1:medkit_army:600:3:4'],
    params: [
      { name: 'stash_name', type: 'stash_name', required: true, label: 'Stash Location', editor: { kind: 'stash_picker' } },
      { name: 'item', type: 'item_section', required: true, label: 'Item to Retrieve', editor: ITEM_PICKER_PANEL_EDITOR },
      { name: 'timeout', type: 'number', required: true, label: 'Timeout (s)', min: 30 },
      { name: 'success_turn', type: 'number', required: true, label: 'Success Turn', min: 2, editor: TURN_REFERENCE_EDITOR },
      { name: 'fail_turn', type: 'number', required: true, label: 'Fail Turn', min: 2, editor: TURN_REFERENCE_EDITOR },
    ],
