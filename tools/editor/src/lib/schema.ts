...
export const OUTCOME_SCHEMAS: CommandSchema[] = [
  ...
  },
  {
    ...
    helpText:
      ...
      'Use this path when you want a custom NPC to stay locked to a location: turn roaming off in the template builder and the runtime will bind that NPC to a compatible stalker job on the chosen smart terrain. ' +
      'Not every smart terrain has a valid fixed-position stalker job, so choose a human-capable smart terrain for non-roamable NPCs. ' +
...

  {
    ...
    label: 'Task: Delivery',
    description: 'Courier delivers item to player; player must bring it to destination',
    category: 'Tasks',
    helpText: 'A courier NPC spawns near the player with the specified item. The player must take it and deliver it to the destination smart terrain. The conversation resumes at the success or fail turn based on the outcome.',
    examples: ['panda_task_delivery:medkit_army:st_esc_smart_terrain_5_4_name:600:3:4'],
    params: [
      { name: 'item', type: 'item_section', required: true, label: 'Item to Deliver', editor: ITEM_PICKER_PANEL_EDITOR },
