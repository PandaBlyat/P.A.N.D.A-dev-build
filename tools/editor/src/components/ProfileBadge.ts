...
function buildNextGoalsPanel(profile: UserProfile): HTMLElement {
  ...
  subtitle.textContent = nextTargets.length > 0
    ? `Suggested achievement unlocks based on your current progression (${Math.min(nextTargets.length, 5)} available).`
    : 'No suggested goals right now — new achievements will appear as you progress.';
  titleWrap.append(title, subtitle);
  header.appendChild(titleWrap);

  const list = document.createElement('div');
  list.className = 'profile-achievement-next-list profile-next-goals-list';

  if (nextTargets.length === 0) {
    const empty = document.createElement('div');
    empty.className = 'profile-achievement-next-card profile-surface-card';
    const emptyLabel = document.createElement('div');
    emptyLabel.className = 'profile-achievement-next-meta';
    emptyLabel.textContent = 'No goals queued';
    const emptyTitle = document.createElement('div');
    emptyTitle.className = 'profile-achievement-next-name';
    emptyTitle.textContent = 'You are caught up';
    const emptyDesc = document.createElement('p');
    emptyDesc.className = 'profile-achievement-next-desc';
    ...
    nextTargets.slice(0, 3).forEach(({ achievement, reason }) => {
      ...
      goalCard.className = `profile-achievement-next-card profile-surface-card profile-achievement-next-card-${achievement.tier}`;
      goalCard.title = `${achievement.name} — ${achievement.description}`;

      const goalTop = document.createElement('div');
      goalTop.className = 'profile-achievement-next-top';
      const goalIcon = document.createElement('span');
      goalIcon.className = 'profile-achievement-next-icon';
      goalIcon.textContent = achievement.icon;
      const copy = document.createElement('div');
      copy.className = 'profile-achievement-next-copy';
      const goalTitle = document.createElement('div');
      goalTitle.className = 'profile-achievement-next-name';
      goalTitle.textContent = achievement.name;
      copy.appendChild(goalTitle);
      goalTop.append(goalIcon, copy);

      const goalMeta = document.createElement('div');
      goalMeta.className = 'profile-achievement-next-meta';
      ...
      goalDesc.className = 'profile-achievement-next-desc';
