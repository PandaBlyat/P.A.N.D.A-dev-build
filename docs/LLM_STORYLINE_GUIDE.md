...
| `gender` | `"male"` or `"female"`. Female uses woman voice only. Female model comes from female-compatible `outfit`, `visual`, or `visualPreset`; no automatic Hip or male outfit visual fallback. |
...
| `outfit` | Armor item section. For female NPCs, editor limits choices to known Dux-compatible female outfits and engine maps selected outfit to a matching female visual. |
